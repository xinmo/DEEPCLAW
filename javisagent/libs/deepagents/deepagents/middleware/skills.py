...
_VERSIONED_SKILL_DIRECTORY_PATTERN = re.compile(r"^(?P<name>.+)-(?P<version>\d+(?:\.\d+)*)$")
...
class SkillsState(AgentState):
    ...
    skills_source_signature: NotRequired[Annotated[str, PrivateStateAttr]]
    """Signature of the configured skill sources used to populate `skills_metadata`."""
...
class SkillsStateUpdate(TypedDict):
    ...
    skills_source_signature: str
    """Signature of the configured skill sources associated with `skills_metadata`."""
...
def _load_skill_from_directory(
    ...
    """Attempt to load a single skill from a directory path.

    This supports using a source path that points directly at one skill
    directory instead of a parent directory containing many skills.
    """
    skill_dir = PurePosixPath(directory_path.rstrip("/"))
...
async def _aload_skill_from_directory(
    backend: BackendProtocol,
    directory_path: str,
) -> SkillMetadata | None:
    """Async version of `_load_skill_from_directory`."""
    skill_dir = PurePosixPath(directory_path.rstrip("/"))
    skill_md_path = str(skill_dir / "SKILL.md")
    responses = await backend.adownload_files([skill_md_path])
    if not responses:
        return None
...
def _validate_skill_name(name: str, directory_name: str) -> tuple[bool, str]:
    ...
    - Must not contain consecutive `--`
    - Must match the parent directory name containing the `SKILL.md` file.
      For backwards compatibility, a versioned directory in the form
      `<name>-<semver>` is also accepted.
    ...
    versioned_match = _VERSIONED_SKILL_DIRECTORY_PATTERN.fullmatch(directory_name)
    if versioned_match and versioned_match.group("name") == name:
        return True, ""
...
def _list_skills(backend: BackendProtocol, source_path: str) -> list[SkillMetadata]:
    ...
    """
    single_skill = _load_skill_from_directory(backend, source_path)
...
async def _alist_skills(backend: BackendProtocol, source_path: str) -> list[SkillMetadata]:
    ...
    Returns:
        List of skill metadata from successfully parsed `SKILL.md` files
    """
    single_skill = await _aload_skill_from_directory(backend, source_path)
    if single_skill is not None:
        return [single_skill]
...
技能遵循**渐进式披露**模式 - 你在上面看到它们的名称和描述，但只在需要时阅读完整说明：

1. **识别技能何时适用**：检查用户的任务是否与技能的描述匹配
2. **阅读技能的完整说明**：使用上面技能列表中显示的路径
3. **遵循技能的说明**：SKILL.md 包含分步工作流程、最佳实践和示例
...
记住：技能使你更有能力和一致性。如有疑问，检查任务是否存在技能！
"""
...
class SkillsMiddleware(AgentMiddleware[SkillsState, ContextT, ResponseT]):
        ...
        *,
        ...
        Args:
            ...
            system_prompt_template: Prompt template injected into the system
                message. Must include `{skills_locations}` and `{skills_list}`
                placeholders.
        """
    ...
    def _get_sources_signature(self) -> str:
        """Return a stable signature for the configured skill sources."""
    ...
    def before_agent(self, state: SkillsState, runtime: Runtime, config: RunnableConfig) -> SkillsStateUpdate | None:  # ty: ignore[invalid-method-override]
        ...
        """
        current_signature = self._get_sources_signature()
        if (
            "skills_metadata" in state
            and state.get("skills_source_signature") == current_signature
        ):
            return None
        ...
        return SkillsStateUpdate(
            ...
            skills_source_signature=current_signature,
        )

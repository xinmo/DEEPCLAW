...
SUMMARIZATION_SYSTEM_PROMPT = """## 压缩对话工具 `compact_conversation`

你可以访问 `compact_conversation` 工具。此工具刷新你的上下文窗口以减少上下文膨胀和成本。

你应该在以下情况下使用该工具：
- 用户要求转到一个全新的任务，之前的上下文可能不相关。
- 你已经完成提取或综合结果，不再需要之前的工作上下文。
"""
...
def create_summarization_middleware(
    ...
    *,
    ...
    Args:
        ...
        backend: Backend instance or factory for persisting conversation history.
        summary_prompt: Optional prompt template override used for generating summaries.
...
def create_summarization_tool_middleware(
    ...
    *,
    ...
    Args:
        ...
        backend: Backend instance or factory for persisting conversation history.
        system_prompt: Optional system prompt override appended when teaching the
            model how to use `compact_conversation`.
    ...
    return SummarizationToolMiddleware(
        summarization,
        system_prompt=system_prompt or SUMMARIZATION_SYSTEM_PROMPT,
    )
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            system_prompt: System prompt text appended on model calls to guide
                `compact_conversation` usage.
        """

...
class FilesystemBackend(BackendProtocol):
    ...
    def _resolve_path(self, key: str) -> Path:
        ...
        if self.virtual_mode:
            ...
            full: Path
            windows_virtual_match = re.match(r"^/([a-zA-Z])(?:/(.*))?$", vpath)
            current_drive = self.cwd.drive.rstrip(":").upper()
            if (
                os.name == "nt"
                and windows_virtual_match
                and current_drive
                and windows_virtual_match.group(1).upper() == current_drive
            ):
                drive = windows_virtual_match.group(1).upper()
                suffix = windows_virtual_match.group(2) or ""
                full = Path(f"{drive}:/{suffix}").resolve()
            else:

...
class TestFilesystemMiddleware:
    ...
    def test_read_file_text_returns_tool_message_with_source_path(self):
        """Text reads should also preserve the source file path for downstream UIs."""

        state = FilesystemState(
            messages=[],
            files={
                "/app/main.tsx": FileData(
        ...
        read_file_tool = next(tool for tool in middleware.tools if tool.name == "read_file")
        result = read_file_tool.invoke({"file_path": "/app/main.tsx", "runtime": runtime})

        assert isinstance(result, ToolMessage)
        assert result.name == "read_file"
        assert result.tool_call_id == "text-read-1"
        assert result.additional_kwargs["read_file_path"] == "/app/main.tsx"
        assert isinstance(result.content, str)
        assert "import React" in result.content

...
class TestWindowsPathHandling:
    ...
    @pytest.mark.skipif(os.name != "nt", reason="Windows-only virtual drive mapping")
    def test_windows_drive_virtual_path_resolves_within_root(self, tmp_path: Path):
        """A /c/... virtual path should map to the real Windows path when still under root."""
        be = FilesystemBackend(root_dir=str(tmp_path), virtual_mode=True)

        drive = tmp_path.drive.rstrip(":").lower()
        relative = tmp_path.relative_to(tmp_path.anchor).as_posix()
        virtual_path = f"/{drive}/{relative}/written.txt"

        result = be.write(virtual_path, "windows path content")

        assert result.error is None
        assert (tmp_path / "written.txt").exists()

    @pytest.mark.skipif(os.name != "nt", reason="Windows-only virtual drive mapping")
    def test_windows_drive_virtual_path_outside_root_is_rejected(self, tmp_path: Path):
        """A /c/... virtual path outside the configured root should still be blocked."""
        be = FilesystemBackend(root_dir=str(tmp_path), virtual_mode=True)
        drive = tmp_path.drive.rstrip(":").lower()

        with pytest.raises(ValueError, match="outside root directory"):
            be.read(f"/{drive}/Windows/System32/drivers/etc/hosts")
